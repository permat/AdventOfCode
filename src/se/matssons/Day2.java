...
//    };
    ...
    public static void calculateCorruptionChecksum() {
        int checksum = Arrays.stream(matrix).map(r -> {
            OptionalInt min = Arrays.stream(r).min();
            OptionalInt max = Arrays.stream(r).max();
            return max.getAsInt() - min.getAsInt();
        }).mapToInt(i -> i).sum();

        System.out.println("Day 2: Corruption checksum = " + checksum + ".");
    }
